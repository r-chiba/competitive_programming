...
int bfs(P s, P t, int n)
{
    ...
    while(!q.empty()) {
        ...
        if (p.se.fi < 0 || H <= p.se.fi || p.se.se < 0 || W <= p.se.se) {
            continue;
        }
        if (m[p.se.fi][p.se.se] == 'X' || m[p.se.fi][p.se.se] == n) {
            continue;
        }
        m[p.se.fi][p.se.se] = n;
        REP (i, 4) {
            P n = mp(p.se.fi + dx[i], p.se.se + dy[i]);
            q.emplace(p.fi+1, n);
        }
    }
    return INF;
...
int main()
{
    ...
    REP (i, H) REP (j, W) {
        cin >> m[i][j];
        if (m[i][j] == 'S') s = mp(i, j);
        else if ('1' <= m[i][j] && m[i][j] <= '1'+N-1) f[m[i][j]-'1'] = mp(i, j);
    }
    solve();
    return 0;
}
