...
void solve()
{
    ...
    //cerr << endl;
    REP (i, K) {
        int pi = v[i].fi.fi, pj = v[i].fi.se;
        int qi = v[i].se.fi, qj = v[i].se.se;
        int nj = sj[qi][qj]
                    - (pi > 0 ? sj[pi-1][qj] : 0)
                    - (pj > 0 ? sj[qi][pj-1] : 0)
                    + (pi > 0 && pj > 0 ? sj[pi-1][pj-1] : 0);
        int no = so[qi][qj]
                    - (pi > 0 ? so[pi-1][qj] : 0)
                    - (pj > 0 ? so[qi][pj-1] : 0)
                    + (pi > 0 && pj > 0 ? so[pi-1][pj-1] : 0);
        int ni = (qi - pi + 1) * (qj - pj + 1) - nj - no;
        cout << nj << " " << no << " " << ni << endl;
    }
}
