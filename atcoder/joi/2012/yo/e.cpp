...
int H, W;
int m[110][110];
constexpr int dxe[] = {1, 0, -1, -1, -1, 0};
constexpr int dye[] = {0, 1, 1, 0, -1, -1};
constexpr int dxo[] = {1, 1, 0, -1, 0, 1};
constexpr int dyo[] = {0, 1, 1, 0, -1, -1};
...
int bfs(int x, int y) {
    ...
    while (!q.empty()) {
        ...
        int a = 0;
        REP (i, 6) {
            int nx = cx + (cy % 2 == 0 ? dxe[i] : dxo[i]);
            int ny = cy + (cy % 2 == 0 ? dye[i] : dyo[i]);
