...
void solve()
{
    FOR (i, 1, D) {
        REP (j, N) {
            REP (k, N) {
                REP (l, N) {
                    if (temp[i+1] < c[l].se.fi || c[l].se.se < temp[i+1])
                        continue;
                    if (temp[i] < c[k].se.fi || c[k].se.se < temp[i])
                        continue;
                    dp[i+1][l][k] = max(dp[i+1][l][k], dp[i][k][j] + abs(c[l].fi - c[k].fi));
                }
            }
        }
    }
    int ret = 0;
    REP (i, N) REP (j, N) ret = max(ret, dp[D][i][j]);
    cout << ret << endl;
}
...
int main()
{
    ...
    cin >> D >> N;
    FOR (i, 1, D+1) cin >> temp[i];
    REP (i, N) cin >> c[i].se.fi >> c[i].se.se >> c[i].fi;
    solve();
    return 0;
}
