...
static inline double dist(P &x, P &y)
{
    double dd = (x.fi-y.fi)*(x.fi-y.fi) + (x.se-y.se)*(x.se-y.se);
    return sqrt(dd);
}
...
void solve()
{
    REP (i, M) {
        ...
        REP (j, N) {
            d[i] = min(d[i], dist(p[i], b[j].se) - b[j].fi);
        }
    }
    REP (i, M) {
        FOR (j, i+1, M) {
    ...
    REP (i, N) {
        ans = min(ans, static_cast<double>(b[i].fi));
    }
    REP (i, M) {
        ans = min(ans, d[i]);
    }
...
int main()
{
    ...
    cin >> N >> M;
    REP (i, N) cin >> b[i].se.fi >> b[i].se.se >> b[i].fi;
