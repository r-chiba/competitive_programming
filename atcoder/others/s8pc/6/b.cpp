...
static inline ll minTime(ll ent, ll exit, ll a, ll b)
{
    ll ans = LINF;
    ans = min(ans, abs(ent - a) + abs(a - b) + abs(b - exit));
    ans = min(ans, abs(ent - b) + abs(b - a) + abs(a - exit));
    return ans;
}
...
void solve()
{
    ...
    FOR (i, 0, psz) {
        FOR (j, 0, psz) {
            ...
            ll total = 0;
            REP (k, N) {
                total += minTime(ent, exit, p[k].fi, p[k].se);
            }
            ans = min(ans, total);
        }
    }
