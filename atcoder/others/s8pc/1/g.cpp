...
struct Edge {
    int to_;
    ll d_, t_;
    Edge(int to, ll d, ll t): to_(to), d_(d), t_(t) {}
};

int N, M;
vector<Edge> g[16];
unordered_map<pair<pair<int, unsigned>, ll>, LP> m;

LP rec(int v, unsigned b, ll t)
{
    auto p = mp(mp(v, b), t);
    if (b == (1u<<N)-1) {
        //cerr << p << " " << mp(t, (v == 0 ? 1 : 0)) << endl;
        return mp(t, (v == 0 ? 1 : 0));
    }
    if (m.find(p) != m.end()) {
        //cerr << p << " " << m[p] << endl;
    ...
    ll tt = LINF, nn = 0;
    for (auto &e: g[v]) {
        if (t + e.d_ > e.t_ || (b&(1u<<e.to_)) != 0) continue;
        auto pp = rec(e.to_, b|(1u<<e.to_), t+e.d_);
        if (pp.se > 0) {
            if (tt > pp.fi) {
                tt = pp.fi;
                nn = pp.se;
...
void solve()
{
    ll t = LINF, n = 0;
    for (auto &e: g[0]) {
        if (e.d_ > e.t_) continue;
        auto p = rec(e.to_, 1u<<e.to_, e.d_);
        if (p.se > 0) {
            if (t > p.fi) {
                t = p.fi;
                n = p.se;
...
int main()
{
    ...
    REP (i, M) {
        ...
        cin >> s >> to >> d >> t;
        --s; --to;
        g[s].emplace_back(to, d, t);
        g[to].emplace_back(s, d, t);
    }
    //for (int i = 0; i < N; ++i) {
    //    cerr << i << endl;
    //    for (auto &e: g[i]) {
    //        cerr << "\t" << e.to_ << " " << e.d_ << " " << e.t_ << endl;
    //    }
    //}
    solve();
    return 0;
