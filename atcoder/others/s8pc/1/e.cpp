...
void solve()
{
    FOR (i, 1, N) d[i] = pow(a[i-1], a[i]);
    FOR (i, 1, N) ds[i] = (ds[i-1] + d[i]) % MOD;
    ll ans = 0;
    FOR (i, 1, Q+2) {
        ll dist = mod(ds[max(c[i], c[i-1])] - ds[min(c[i], c[i-1])], MOD);
        //cerr << mp(c[i-1], c[i]) << " " << dist << endl;
        ans += dist;
        ans %= MOD;
    }
    cout << ans << endl;
