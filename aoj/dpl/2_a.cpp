...
int V, E;
int g[15][15];
unordered_map<P, int> m;

int rec(unsigned b, int cur)
{
    b |= 1<<cur;
    //cerr << "[" << bitset<15>(b) << ", " << cur << "]" << endl;
    if (b == (1<<V)-1) {
        //cerr << bitset<15>(b) << "!" << cur << "!" << g[cur][0] << endl;
        return g[cur][0];
    }
    if (m.find(mp(b, cur)) != m.end()) {
        return m[mp(b, cur)];
    }
    int ret = INF;
    REP (i, V) {
        if ((b & (1<<i)) == 0 && g[cur][i] != INF) {
            ret = min(ret, rec(b, i) + g[cur][i]);
        }
    }
    //cerr << bitset<15>(b) << " " << cur << " " << ret << endl;
    return m[mp(b, cur)] = ret;
}
